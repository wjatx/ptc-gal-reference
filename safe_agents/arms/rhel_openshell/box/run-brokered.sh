...
# ec2-woken box — while KEEPING the netns as a defense-in-depth process-isolation layer. The netns
# now FORWARDS to the broker service (agent-netns-setup.sh) instead of blackholing to a co-located
# stub. Re-derived clean from safe_agents/arms/ec2/box/run-brokered.sh (no development-harness dependency).
#
...
#
# Env contract (from /etc/safe-agents/agent.env, written by bootstrap.sh — do NOT rename):
...
#   AGENT_RUN_USER        unprivileged user the confined turn drops to (default dev)
...
if [ "${SA_CONFINED_PHASE:-0}" = "1" ]; then
    ...
    toolcall_out="$(curl -sS -m20 -X POST "http://${BROKER}:${TOOL_PORT}/call" \
        ...
        -d "{\"tool\":\"github\",\"op\":\"whoami\",\"args\":{},\"idempotency_key\":\"rhel-${RUN_ID:-run}\"}" \
        2>&1)"
...
RUN_ID="${RUN_ID:-rhel-$(date -u +%Y%m%dT%H%M%SZ)}"
export RUN_ID
...
echo "=== rhel run: agent=${AGENT_NAME} run=${RUN_ID} broker=${BROKER} profile=${SA_PROFILE:-autonomous} ==="
...
echo "=== run record → ${AGENT_RUNS_TABLE} (arm=rhel-openshell) ==="
...
if [ "$status" = "ok" ]; then
    echo "RHEL_RUN: PASS (run=${RUN_ID})"; exit 0
else
    echo "RHEL_RUN: FAIL (run=${RUN_ID})"; exit 1
fi
